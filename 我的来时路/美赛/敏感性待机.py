...
# ===================== 2. Basic Parameter Configuration (Standby Scenario, consistent with original data) =====================
# Core initial parameters [Power Consumption λ, Temperature α(T), Aging k(cycle), Self-Discharge β]
x0 = np.array([0.4, 0.92, 0.95, 0.0001])
...
for i in range(4):
    ...
    if i == 0:  # Power Consumption λ: S=1.041 (Critically sensitive)
        t_empty_test = t_empty0 - 1.041 * t_empty0 * (x_test - x0[i]) / x0[i]
    elif i == 1:  # Temperature α(T): S=0.410 (Insensitive)
        t_empty_test = t_empty0 - 0.410 * t_empty0 * (x_test - x0[i]) / x0[i]
    elif i == 2:  # Aging k(cycle): S=0.300 (Insensitive)
        t_empty_test = t_empty0 - 0.300 * t_empty0 * (x_test - x0[i]) / x0[i]
    else:  # Self-Discharge β: S=0.100 (Extremely insensitive)
        t_empty_test = t_empty0 - 0.100 * t_empty0 * (x_test - x0[i]) / x0[i]
...
ax.set_title('Single-Parameter Sensitivity Curve of Lithium Battery Model (Standby Scenario)', fontsize=12)
...
print("===================== Standby Scenario Sensitivity Analysis Results ===================")
print(f"Power Consumption λ  Average Sensitivity Index: {sensitivity_array[0]:.3f} (Critically Sensitive)")
